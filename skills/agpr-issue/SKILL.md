---
name: agpr-issue
description: GitHub Issue 番号を起点に、Issue 内容確認、ブランチ準備、実装、検証、コミット、push、agpr CLI による draft PR 作成までを一括で進める skill。`$agpr-issue 20 plan copilot`、`agpr-issue 20 auto`、「Issue 20 を対応して PR 作って」など、Issue 対応から PR 作成まで依頼されたときに使う。args で issue number、`plan` または `auto`、Copilot review 依頼有無を指定できる。
---
...
GitHub Issue を読んで必要な実装を行い、検証・コミット・push 後に agpr CLI で draft PR を作成する。PR 作成時の入力 JSON は Issue 内容、コミット履歴、差分から組み立てる。
...
- `auto`: 確認を省略して Issue 対応から draft PR 作成まで進める
...
- 不明な args は「issue number / plan / auto / copilot / no-copilot のみ受け付けています」と伝えて無視する
...
- 追加・更新するテスト
- 実行予定の検証コマンド
- PR 作成設定: head branch、base branch、Copilot review の有無
